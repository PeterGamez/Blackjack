...
export default function Home() {
  ...
  const buttonStyle = (name: string) => {
    ...
    switch (name) {
      // 1️⃣ Shop
      case "shop":
        background = "linear-gradient(90deg, #C99A3F 0%, #E6B85C 50%, #F2C879 100%)";
        break;
      ...
      // 3️⃣ Gambling
      case "gambling":
        background = "linear-gradient(90deg, #F2C879 0%, #E6B85C 50%, #C99A3F 100%)";
        break;
    ...
    return {
      ...
      transform: hovered === name ? "skewX(-45deg) scale(1.05)" : "skewX(-45deg)",
      cursor: "pointer",
      transition: "all 0.2s ease",
      whiteSpace: "nowrap",
      marginLeft: "-6px",
      boxShadow: hovered === name ? "0 8px 20px rgba(0,0,0,0.25)" : "0 2px 6px rgba(0,0,0,0.15)",
    };
  };
    ...
    <div
      ...
      <div
        ...
        <div
          ...
          {menuItems.map((item) => (
            <button key={item.key} onClick={() => router.push(item.path)} onMouseEnter={() => setHovered(item.key)} onMouseLeave={() => setHovered(null)} style={buttonStyle(item.key)}>
              <span style={{ display: "inline-block", transform: "skewX(45deg)" }}>{item.label}</span>
            </button>
          ))}
        </div>
      </div>
